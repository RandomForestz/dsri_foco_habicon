...
hydrology_path <- c("data/input_raw/Hydrology/Hydrology.shp")
nlcd_path <- c("data/input_raw/nlcd_tcc_CONUS_2021_v2021-4/nlcd_tcc_conus_2021_v2021-4.tif")
...
process_preds <- function(predictor_path, template, resolution, save, output_path) {
  ...
  if (grepl("\\.shp$", predictor_path, ignore.case = TRUE)) {
    ...
    output_object <- list(name = file_name, object = rast_shapefile)
    ...
    # Store the processed object with the filename
    output_object <- list(name = file_name, object = raster_file)
...
hydrology_processed<- process_preds(hydrology_path, template, resolution, save=TRUE, output_path)
nlcd_processed <- process_preds(nlcd_path, template, resolution, save=TRUE, output_path)
...
tm_shape(template) +
  ...
  tm_shape(nlcd_processed[[2]]) + # Access the first SpatRaster object in the list
  tm_raster() +
  tm_shape(hydrology_processed[[2]]) + # Access the first SpatRaster object in the list
  tm_raster()
```
